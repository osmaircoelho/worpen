...
# Count Results
$select_db_count = $connect->prepare("SELECT COUNT(*) FROM worpen_module WHERE (name LIKE :search OR date_create LIKE :search OR url LIKE :search) AND plataform LIKE :plataform");
...
# Search the Database
$select_db = $connect->prepare("SELECT * FROM worpen_module WHERE (name LIKE :search OR date_create LIKE :search OR url LIKE :search) AND plataform LIKE :plataform ORDER BY id DESC LIMIT {$number_result_page}, {$results_for_page}");
...
while ($result = $select_db->fetch(PDO::FETCH_ASSOC)) {
  echo "<tr>
          ...
          <td class=\"text-center\">
            <a href=\"?mod=admin&pg=edit_module&id={$result['id']}\" class=\"btn btn-default\"><span class=\"glyphicon glyphicon-pencil\" aria-hidden=\"true\"></span></a>
          </td>
        </tr>";
}
