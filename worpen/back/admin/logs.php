...
# Count Results
$select_db_count = $connect->prepare("SELECT COUNT(*) FROM worpen_log WHERE user LIKE :search OR date_access LIKE :search OR ip LIKE :search AND plataform LIKE :plataform");
...
# Search the Database
$select_db = $connect->prepare("SELECT * FROM worpen_log WHERE user LIKE :search OR date_access LIKE :search OR ip LIKE :search AND plataform LIKE :plataform ORDER BY id DESC LIMIT {$number_result_page}, {$results_for_page}");
