...
if (!$search_user) {
  ...
  # Count Results
  $select_db_count = $connect->prepare("SELECT COUNT(*) FROM worpen_users");
  ...
  # Search the Database
  $select_db = $connect->prepare("SELECT * FROM worpen_users ORDER BY id DESC LIMIT {$number_result_page}, {$results_for_page}");
  $select_db->execute();
  ...
  while ($result = $select_db->fetch(PDO::FETCH_ASSOC)) {
    echo "<tr>
            ...
            <td class=\"text-center\"><a href=\"?mod=admin&pg=view_user&id={$result['id']}\" class=\"btn btn-default\"><i class=\"fa fa-eye\"></i></a> <a href=\"?mod=admin&pg=edit_user&id={$result['id']}\" class=\"btn btn-default\"><i class=\"fa fa-pencil\"></i></a></td>
          </tr>";
  }
